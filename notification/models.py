...
class Notice(models.Model):
    ...
    object_type = models.ForeignKey(ContentType, null=True)
    ...
    def get_message_url(self):
        return 'notification/%s/notice.html' % self.notice_type
    
    def get_the_object(self):
        return self.object_type.get_object_for_this_type(pk=self.object_id)
    
    def get_comments(self):
        return self.object_type.get_object_for_this_type(pk=self.object_id).comment.all().order_by('-date')
...
def send_now(users, label, extra_context=None, on_site=True, sender=None, object_type=None, object_id=0):
    """
    ...
    for user in users:
        ...
        notice = Notice(recipient=user, message=messages['notice.html'],
            notice_type=notice_type, on_site=on_site, sender=sender, object_type=object_type, object_id=object_id)
        notice.save()
