...
from synclee.notification.models import *
from synclee.notification.decorators import basic_auth_required, simple_basic_auth_callback
from synclee.notification.feeds import NoticeUserFeed
...
def notices(request):
    ...
    mail_notices = notices.filter(notice_type__label='new_message')
    accept_notices = notices.filter(notice_type__label__in=['work_accept', 'club_accept'])
    apply_notices = notices.filter(notice_type__label__in=['work_apply', 'club_apply'])
    invite_notices = notices.filter(notice_type__label__in=['work_invite', 'club_invite'])
    reply_notices = notices.filter(notice_type__label__in=['new_reply'])
    
    cata = request.GET.get('cata')
    if not cata:
        cata = 'rep'
    if cata == 'rep':
        object = reply_notices.filter(unseen=False)
    if cata == 'app':
        object = apply_notices.filter(unseen=False)
    if cata == 'inv':
        object = invite_notices.filter(unseen=False)
    if cata == 'agr':
        object = accept_notices.filter(unseen=False)
        
    
    paginator = Paginator(object, 10) # Show 25 contacts per page
    ...
    response_html = "notification/notices_%s.html" % cata
    
    return render_to_response(response_html, {
        "notices": notices,
        "mail": mail_notices,
        "accept": accept_notices,
        "invite": invite_notices,
        "apply": apply_notices,
        "reply": reply_notices,
        "notices": n,
        "cata": cata
...
def refresh_notices(request):
    ...
    response_html = "notification/%s_old.html" % cata
...
def single(request, id, mark_seen=True):
    ...
    if request.user == notice.recipient:
        if mark_seen and notice.unseen:
            request.user.get_profile().unread_notes -= 1
